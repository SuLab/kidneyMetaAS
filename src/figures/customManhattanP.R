...
manhattan <- function(x, chr="CHR", bp="BP", p="P.value", snp="SNP", 
                      col=c("gray10", "gray60"), chrlabs=NULL,
                      suggestiveline=-log10(5e-60), genomewideline=-log10(5e-80), 
...
#   def_args <- list(xaxt='n', bty='n', xaxs='i', yaxs='i', las=1, pch=20,
#                    xlim=c(xmin,xmax), ylim=c(0, 6.5),
#                    xlab=xlabel, ylab=expression(-log[10](italic(Permuted_P))))
  ...
  if (!is.null(highlight)) {
    ...
    with(d.highlight, points(pos, logp, col="red3", pch=19, cex=2, lwd=6, ...))
    with(d.highlight, text(pos, logp, labels=paste("        ", highlight, sep=""), col="blue3", cex=2, pos=3, ...)) 
  }
